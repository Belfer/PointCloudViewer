...
bool ImGui_ImplGlfwGL3_CreateFontsTexture() {
  ...
  io.Fonts->GetTexDataAsRGBA32(
      &pixels, &width, &height); // Load as RGBA 32-bits (75% of the memory is
                                 // wasted, but default font is so small)
                                 // because it is more likely to be compatible
                                 // with user's existing shaders. If your
                                 // ImTextureId represent a higher-level concept
                                 // than just a GL texture id, consider calling
                                 // GetTexDataAsAlpha8() instead to save on GPU
                                 // memory.
    ...
    if (io.WantMoveMouse) {
      glfwSetCursorPos(g_Window, (double)io.MousePos.x,
                       (double)io.MousePos.y); // Set mouse position if
                                               // requested by io.WantMoveMouse
                                               // flag (used when
                                               // io.NavMovesTrue is enabled by
                                               // user and using directional
                                               // navigation)
    } else {
